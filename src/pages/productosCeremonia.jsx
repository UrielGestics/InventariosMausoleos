...
export const ProductosCeremonia = () => {
    ...
    const obtenerarticulosceremonias = () =>{
        fetch(`${apiURL}ceremonias.php?tipo=obtenerArticulosCeremoniasActivos&ID_Ceremonia=${id}`)
        .then(async(resp) =>{
    ...
    const requestSearch = (texto) => {
      ...
      const agregarArticuloCeremonia = () =>{
        Swal.fire({
            allowOutsideClick: false,
            title: `Agregar Arituclo A ${nombre}`,
            html: `<label>Nombre Del Articulo</label>
            <br />
            <input type="text" id="nArtCer" class="swal2-input" placeholder="Nombre Del Articulo">
            `,
            ...
            if(result.isDismissed){
              Swal.fire('Accion Cancelada', 'No Se Agrego El Articulo', 'error')
                ...
                const nArtCer = Swal.getPopup().querySelector('#nArtCer').value
                if (!nArtCer ) {
              ...
              Swal.fire({
                ...
                willClose: () => {
                  ...
                  formData.append("tipo","agregarArtCeremonia")
                  formData.append("nArtCer",nArtCer)
      ...
      const desHabilitarArtCeremonia =(ID_CeremoniasXArticulo,Nombre_Articulo) =>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
              ...
              Swal.fire({
                ...
                let formData = new FormData();
                formData.append("tipo","desHabilitarArtCeremonia");
                ...
                .then(async(resp) =>{
                    ...
                    if(estatus){
                        ...
                        Swal.fire('Error', `Hubo un error al Deshabilitar El Articulo ${Nombre_Articulo}`, 'error')
                    }
                })
            } else if (result.isDenied) {
              Swal.fire('Accion Cancelada', 'No Deshabilito El Articulo', 'error')
            }
          })
    }

    const editarArtProveedor = (ID_CeremoniasXArticulo,Nombre_Articulo) =>{
        Swal.fire({
            allowOutsideClick: false,
            title: `Modificar Articulo ${Nombre_Articulo}`,
            html: `<label>Nombre Del Articulo</label>
            <br />
            <input type="text" id="nArtCer" class="swal2-input" value='${Nombre_Articulo}' placeholder="Nombre Del Articulo">
            `,
            ...
            if(result.isDismissed){
              ...
              Swal.fire({
                ...
                willClose: () => {
                  ...
                  //Hacer Petición API
                  let formData = new FormData();
                  formData.append("tipo","modificarArtCeremonia")
                  formData.append("id",ID_CeremoniasXArticulo)
                  formData.append("nArtCer",nArtCer)
    ...
    return (
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 1 }}>
        ...
        <>
        <Button onClick={agregarArticuloCeremonia} title='Agregar' key='AgregarArt' variant="contained" color='success' style={{float: 'right', marginBottom: '5px', color: 'white'}} size="small"><i className="bi bi-plus-circle" style={{marginRight : '10px'}}></i> Agregar Articulo</Button>
        ...
        <TableBody>
          ...
          :(articulosceremonias!=undefined) ? articulosceremonias.slice(pg * rpg, pg * rpg + rpg).map(({ID_CeremoniasXArticulo,Nombre_Articulo}) =>(
            ...
            >
              ...
              <Button  onClick={() => editarArtProveedor(ID_CeremoniasXArticulo,Nombre_Articulo)} title='Editar' key={`a${ID_CeremoniasXArticulo}`} variant="contained" color='warning' style={{color: 'white'}} size="small"><i className="bi bi-pencil-square"></i></Button>
              <Button onClick={() => desHabilitarArtCeremonia(ID_CeremoniasXArticulo,Nombre_Articulo)}  title='Baja' key={`b${ID_CeremoniasXArticulo}`} variant="contained" color='error' style={{color: 'white', marginLeft : '10px'}} size="small"><i className="bi bi-trash"></i></Button>
